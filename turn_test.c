...
int main() 
{
  ...
  //  ------  battery example --------------------------------------------------
	...
	while(kb_kbhit()==0)
	{
		l_mot 	= (float)(-dist_tra+2000)/KH4_SPEED_TO_MM_S;
		r_mot 	= (float)(-dist_tra+2000)/KH4_SPEED_TO_MM_S;
		kh4_set_speed(l_mot ,r_mot ,dsPic);
				

		kh4_get_position(&lpos2,&rpos2,dsPic);
		dist_tra = (lpos2 - lpos1)*KH4_PULSE_TO_MM;
	...
	while(kb_kbhit()==0)
	{
		l_mot 	= (float)(0)/KH4_SPEED_TO_MM_S;
		r_mot 	= (float)((22/7)-ang)*10/KH4_SPEED_TO_MM_S;
		kh4_set_speed(l_mot ,r_mot ,dsPic);


		kh4_get_position(&lpos2,&rpos2,dsPic);
		turn	= (rpos2 - rpos1)*KH4_PULSE_TO_MM;
		ang	= turn/(105.4);
	}
	kh4_set_speed(0 ,0,dsPic);
	dist_tra=0;
